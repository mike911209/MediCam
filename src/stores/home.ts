  ...
  const pullNotificationList = async () => {
    try {
      ...
      response.forEach((item: any) => {
        // console.log('item:', item) // Log the response data
        const notify_item: notificationMessage = {
          role: 'assistant',
          status: 'normal',
          content: item.content,
          timestamp: item.timestamp, // Convert timestamp to local string format
        }
        // console.log('res_item:', notify_item) // Log the response data
        notificationMessage.push(notify_item)
      })
    } catch (error) {
